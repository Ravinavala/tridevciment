...
function awl_filter_gallery_shortcode($post_id) {
    ...
    if (isset($pf_gallery_settings['custom-css']))
        ...
        $custom_css = "";
    if (isset($pf_gallery_settings['filter_bg']))
        $filter_bg = $pf_gallery_settings['filter_bg'];
    else
        $filter_bg = "red";
    ...
    if (isset($pf_gallery_settings['filters']))
        $filters = $pf_gallery_settings['filters'];
    else
        $filters = array();
    // ligtbox style
    if (isset($pf_gallery_settings['light-box']))
        $light_box = $pf_gallery_settings['light-box'];
    else
        $light_box = 1;

    //hover effect
    if (isset($pf_gallery_settings['image_hover_effect_type']))
        $image_hover_effect_type = $pf_gallery_settings['image_hover_effect_type'];
    else
    ...
    if (isset($pf_gallery_settings['url_target']))
        $url_target = $pf_gallery_settings['url_target'];
    else
        $url_target = "_new";
    if ($image_hover_effect_type == "2d")
        if (isset($pf_gallery_settings['image_hover_effect_one']))
            $image_hover_effect = $pf_gallery_settings['image_hover_effect_one'];
        else
            $image_hover_effect = "hvr-buzz";
    if ($image_hover_effect_type == "bg")
        if (isset($pf_gallery_settings['image-hover-effect-2']))
            $image_hover_effect = $pf_gallery_settings['image-hover-effect-2'];
        else
            $image_hover_effect = "hvr-bounce-to-top";
    if ($image_hover_effect_type == "br")
        if (isset($pf_gallery_settings['image-hover-effect-3']))
            $image_hover_effect = $pf_gallery_settings['image-hover-effect-3'];
        else
            $image_hover_effect = "hvr-ripple-out";
    if ($image_hover_effect_type == "sg")
        if (isset($pf_gallery_settings['image_hover_effect_four']))
            $image_hover_effect = $pf_gallery_settings['image_hover_effect_four'];
        else
            $image_hover_effect = "hvr-box-shadow-outset";
    if ($image_hover_effect_type == "cl")
        if (isset($pf_gallery_settings['image-hover-effect-5']))
            $image_hover_effect = $pf_gallery_settings['image-hover-effect-5'];
        else
            $image_hover_effect = "hvr-curl-top-left";

    if (isset($pf_gallery_settings['no_spacing']))
        $no_spacing = $pf_gallery_settings['no_spacing'];
    else
        $no_spacing = 1;
    if (isset($pf_gallery_settings['gray_scale']))
        $gray_scale = $pf_gallery_settings['gray_scale'];
    else
        $gray_scale = 0;
    if (isset($pf_gallery_settings['thumbnail_order']))
        $thumbnail_order = $pf_gallery_settings['thumbnail_order'];
    else
        $thumbnail_order = "ASC";
    if (isset($pf_gallery_settings['url_target']))
        $url_target = $pf_gallery_settings['url_target'];
